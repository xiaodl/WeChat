...
public class Fragment_Dicover extends BaseFragment {
    ...
    public void txt_pengyouquan_Click() {
        Route.gotoFeedActivity(getActivity());
    }

    //扫一扫点击事件
    @OnClick(R.id.txt_saoyisao)
    public void txt_saoyisao_Click() {
        Route.gotoZXCode(getActivity());
    }

    //摇一摇点击事件
    @OnClick(R.id.txt_yaoyiyao)
    public void txt_yaoyiyao_Click() {
        Route.gotoCommon(getActivity(), getString(R.string.discover_txt_yaoyiyao));
    }

    //附近的人点击事件
    @OnClick(R.id.txt_nearby)
    public void txt_nearby_Click() {
        Route.startActivity(getActivity(), NearByActivity.class);
    }

    //漂流瓶点击事件
    @OnClick(R.id.txt_piaoliuping)
    public void txt_piaoliuping_Click() {
        Route.gotoCommon(getActivity(), getString(R.string.discover_txt_piaoliuping));
    }

    //购物点击事件
    @OnClick(R.id.txt_shop)
    public void txt_shop_Click() {
        Route.gotoWebView(getActivity(), getString(R.string.discover_txt_shop), Constants.SHOP_URL);
    }

    //游戏点击事件
    @OnClick(R.id.txt_game)
    public void txt_game_Click() {
        Route.gotoWebView(getActivity(), getString(R.string.discover_txt_game), Constants.GAME_URL);
    }
