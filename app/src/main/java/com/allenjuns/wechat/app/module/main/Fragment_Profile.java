...
public class Fragment_Profile extends BaseFragment {
    ...
    public void txt_album_Click() {
        Route.gotoCommon(getActivity(), getString(R.string.profile_txt_album));
    }

    //收藏按钮点击事件
    @OnClick(R.id.txt_collect)
    public void txt_collect_Click() {
        Route.gotoCommon(getActivity(), getString(R.string.profile_txt_collect));
    }

    //钱包按钮点击事件
    @OnClick(R.id.txt_money)
    public void txt_money_Click() {
        Route.gotoWalletActivity(getActivity() );
    }

    //卡包按钮点击事件
    @OnClick(R.id.txt_card)
    public void txt_card_Click() {
        Route.gotoCommon(getActivity(), getString(R.string.profile_txt_card));
    }

    //表情按钮点击事件
    @OnClick(R.id.txt_smail)
    public void txt_smail_Click() {
        Route.gotoCommon(getActivity(), getString(R.string.profile_txt_smail));
    }

    //设置按钮点击事件
    @OnClick(R.id.txt_setting)
    public void setting_Click() {
        Route.gotoSettingActivity(getActivity());
    }

    //个人信息点击事件
    @OnClick(R.id.view_user)
    public void profile_Click() {
        Route.gotoUserCode(getActivity());
    }
}
